...
RYUIP=`sudo docker exec -it mn.dc1_vcpe-1-1-ubuntu-1 ifconfig | grep 10.255.0 | awk '{print $2}'`
echo "----------Iniciando configuracion de Qos para VCPE-1----------"

#Obtencion de direcciones IP de h11 y h12
IP11=`sudo vnx -f ../vnx/nfv3_home_lxc_ubuntu64.xml -M h11 --exe-cli ifconfig | grep 192.168.255 | awk '{print $2}'`
IP12=`sudo vnx -f ../vnx/nfv3_home_lxc_ubuntu64.xml -M h12 --exe-cli ifconfig | grep 192.168.255 | awk '{print $2}'`
...
sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "'$IP11'"}, "actions":{"queue": "0"}}' http://$RYUIP:8080/qos/rules/0000000000000001
sleep 15
echo "Configuracion de rule 2:"
sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "'$IP12'"}, "actions":{"queue": "1"}}' http://$RYUIP:8080/qos/rules/0000000000000001
sleep 5
echo "Get colas:"
